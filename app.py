...
def debug():
    """
    برای عیب‌یابی: نشان می‌دهد وقتی سرور شما مستقیماً به tgju.org وصل می‌شود
    واقعاً چه چیزی دریافت می‌کند. اگر بعد از این تغییرات باز هم قیمت‌ها
    "یافت نشد" بود، این صفحه را باز کنید و خروجی‌اش را برای بررسی بفرستید.
    """
    result = {}
    for name, url in [
        ("homepage", TGJU_BASE),
        ...
        try:
            ...
            result[name] = {
                ...
                "contains_price_dollar_rl_link": "profile/price_dollar_rl" in html,
                "table_tag_count": len(soup.find_all("table")),
                "tr_tag_count": len(soup.find_all("tr")),
