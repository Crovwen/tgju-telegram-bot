...
# API شخص‌ثالث پایدار برای قیمت ارزها (منبع اصلی و قابل‌اعتمادتر از اسکرپ مستقیم).
# این پروژه (BlackIQ/tgju-api) دقیقاً همین مشکل مسدودسازی ربات‌ها روی tgju.org را
# از قبل حل کرده. برای طلا/سکه/تتر که این API پوشش نمی‌دهد، همچنان از اسکرپ
# مستقیم tgju.org به‌عنوان جایگزین استفاده می‌کنیم.
...
def get_currencies_from_api() -> dict:
    ...
    tgju.org که گاهی به‌خاطر سیستم ضدربات سایت، جدول اصلی را برنمی‌گرداند).
    خروجی: دیکشنری slug -> متن قیمت (به ریال، همان‌طور که tgju نمایش می‌دهد).
    اگر درخواست ناموفق باشد، دیکشنری خالی برمی‌گرداند تا کد به روش
    اسکرپ مستقیم به‌عنوان جایگزین برگردد.
    """
    try:
        ...
        log.info("دریافت قیمت ارزها از API شخص‌ثالث موفق بود (%s مورد)", len(result))
        return result
    except Exception as exc:  # noqa: BLE001
        log.warning("خطا در دریافت قیمت ارزها از API شخص‌ثالث: %s", exc)
        return {}
...
def build_message() -> str:
    ...
    # ۱) ارزها: اول از API پایدار شخص‌ثالث، و فقط اگر آن در دسترس نبود
    # به‌عنوان جایگزین از اسکرپ مستقیم صفحه‌ی /currency استفاده می‌کنیم.
    currency_api_data = get_currencies_from_api()
    currency_soup = None if currency_api_data else fetch_soup(f"{TGJU_BASE}/currency")
